...
/**
 * class 라는 개념, 객체지향 프로그래밍(oop)
 * 모듈의 기본 단위
 * 
 * Kotlin 일 경우 class 모듈이 필수 조건이 아니지만
 * 모듈에 class 를 사용하는 이유는
 * 이미 제공해주는 라이브러리의 다른 class 기능을 
 *      상속받아 사용해야 하기 때문에
 ...
 *
 * 상속의 기존 코드의 복사 붙이기와는 완전히 다른 차원의 개념이다
 * 상속은 기존 코드를 변경없이 내 프로젝트에 그래도 적용할 수 있고
 *      만약 기존 코드가 확장(Upgrade) 되면, 내 프로젝트에 적용된 코드도
 *      자동으로 확장(Upgrade) 된다
 *
 * 캡슐화, 추상화
 * 상속을 받는 곳에서 기존의 부모 코드(parents class) 가 어떻게 작동되는지
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btnClick1 = findViewById<Button>(R.id.btn_click1)
        ...
        // setOnClickListener( it -> { 주저리 주저리 } )
        btnClick1.setOnClickListener {
            // Toast Message
            // 잠시 안내 메세지를 보여주고 사라지는 alert
            Toast
                .makeText(
            ...
            Snackbar
                .make(
                    it, // 어디에, 자동으로 Callback 합수의 매개변수로 설정되어 있다
                    "나는 스넥바", // 무엇을
        ...
        // App 이 작동되는 동안에 변화된 값을 그대로 유지하고 있다
         ...
         *
         * Activity 에 비해 Fragment 는 화면을 전환하는데 리소스를 절역하는
         * 방법인데 최근 안드로이드에서 많이 사용하고 권장하는 방법
         ...
         */
         btnClick3.setOnClickListener {
             ...
             val newActivityIntent = Intent(
                 ...
                 // username 이라는 변수를 생성하고 "강한별" 이라는 문자열을 담아서 전달하기
                 putExtra("username","강한별")
                 putExtra("age",25)
             }
             startActivity(newActivityIntent)
        }
        val add1 = findViewById<EditText>(R.id.add1)
        val add2 = findViewById<EditText>(R.id.add2)
        val btnClick4 = findViewById<Button>(R.id.btn_click4)
        btnClick4.setOnClickListener {
            val text1 = add1.text.toString()
            val text2 = add2.text.toString()
            val int1:Int = text1.toInt()
            val int2:Int = text2.toInt()

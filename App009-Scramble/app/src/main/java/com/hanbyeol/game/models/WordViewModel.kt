package com.hanbyeol.game.models

import android.util.Log
import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import com.hanbyeol.game.data.allWordsList

/**
 * 클래스를 만들때 이름 부여 규칙
 * 첫글자는 영문 대문자
 * 두번째부터는 영문 대,소문자, 숫자, _ 등 사용가능
 * 가급적 2단어 이상을 조합하여 명명
 * Under Ber(_) 를 사용하기 보다는
...
class WordViewModel : ViewModel() {
     ...
     * 게임 카운트도 1 증가시키고
     *
     * gameScore 가 10이 되면 "축하합니다" 라는 메세지 출력
     */
    var gameScore = MutableLiveData(0)
    var gameCount = MutableLiveData(0)
    var gameResult = MutableLiveData("")
    ...
    fun nextWord () {
        // 영문 단어를 char(문자) 배열로 변경하고
        val engString = allWordsList.random()
            val rndString = engString.toCharArray()
        // 무작위로 shuffle() 하기, rnd String 데이터 자체가 변경
        rndString.shuffle()

        // char 배열의 문자열(String) 으로 변환시키기

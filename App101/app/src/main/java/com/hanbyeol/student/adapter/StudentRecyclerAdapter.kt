package com.hanbyeol.student.adapter
...
import com.hanbyeol.student.databinding.StudentListItemBinding
 ...
 * helper 클래스
 *
 * 클래스를 상속받았을때 강제로 member 들을 implement 해야하는 경우가 있다
 * 이것은 원본 클래스가
 * 인터페이스 이거나 추상클래스 일 경우이다
 *
 * 클래스를 상속받았는데 오류가 발생하고
 * implements member 메세지가 나오면 method 를 추가하여
 * 코드를 작성해 주어야 한다
 */
class StudentRecyclerAdapter(
    private val studentList : LiveData<List<Student>> // 화면에 보여줄 데이터
) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    /**
     * fragment 나 activity 에서
     * xml 파일과 binding 을 연결하는 역활을 수행하는 함수
     * 여기는 RecyclerView 를 위해서 작동하는 함수
     * 여기서 만든 binding 을 viewHolder 에게 전달하여
     * Data 와 View 를 연결하기
     */
...
//    }
     ...
     * 전달받는다
     * 그리고 상속받은 RecyclerView.ViewHolder 에게
     * binding.root 를 전달해 준다
     */
